...
/*   Updated: 2022/06/09 12:31:44 by bben-yaa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	minimap(t_env *env)
{
	...
	while (y < env->height)
	{
		...
		{
			if (env->map[y][x] == '1')
				ft_draw_square_minimap(env, x * env->minimap.ratio, y * \
				env->minimap.ratio, create_trgb(1, 42, 75, 239));
			else if (env->map[y][x] == '0')
				ft_draw_square_minimap(env, x * env->minimap.ratio, y * \
				env->minimap.ratio, create_trgb(1, 66, 243, 229));
			else
				ft_draw_square_minimap(env, x * env->minimap.ratio, y * \
				env->minimap.ratio, create_trgb(1, 48, 175, 0));
			x++;
		}
		y++;
	}
